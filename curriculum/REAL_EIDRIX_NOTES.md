...
**Deferred to AC-05 Session 2 / real-Eidrix work (NOT built in Session 1):**
- **Parallel tool visibility.** Currently the single-slot slot shows only the most recently started tool. A clean array-within-iteration design (accumulate tools from one tool_use batch, clear on the next batch) would surface all parallel calls simultaneously. Deferred because single-slot ships and works.
- **Continue-from-where-we-left-off.** If a plan hits the iteration cap and finalizes as `failed`, there's no "resume" button. User has to manually re-prompt. Session 2 can wire a resume path via a new `chat` call carrying the prior plan's step state as context.
- **Server-side tool→step annotation.** The cleanest solution to per-step tool attribution would be for `chat.ts` to tag each `eidrix_tool_started` with the `activePlanStepId` the agent most recently marked active. Eliminates client-side heuristics entirely. Session 2.
- **Scroll-back historical chip positioning.** Only the most recent historical plan renders inline in the message stream; older ones are in the Debug tab. Session 2 can interleave all rehydrated chips by timestamp.
