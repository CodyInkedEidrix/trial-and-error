...
**Locked decisions (AC-05 Session 1 — Multi-Turn Agentic Loops):**
- **Plans are first-class DB entities.** `active_plans` table (id, org_id, user_id, conversation_id, triggering_message_id, status enum {running|complete|stopped|failed}, steps jsonb, requested_stop bool, completion_summary, timestamps) with same `is_member_of` + user-scoped RLS pattern as memory_facts. User can't read another member's plans even within an org. Partial index on `status='running'` for cheap rehydrate queries.
- **Server owns the plan lifecycle.** Every status transition happens in `chat.ts` — the client only mirrors SSE events into Zustand. No client speculation, no optimistic state, no "I think this plan is done now" logic on the browser side. Single source of truth, matches the same discipline as messages + memory_facts.
- **Five custom SSE events carry the plan** — three lifecycle (`eidrix_plan_started`, `eidrix_plan_step`, `eidrix_plan_complete`) plus two live-activity (`eidrix_tool_started`, `eidrix_tool_finished`). The activity events fire for every real tool call (not the signaling `emitPlanStep` itself) and power the shimmering per-step sub-line in the UI.
- **`emitPlanStep` is a signaling tool, not a mutation.** Its executor returns a minimal ack; `chat.ts` intercepts the `tool_use` block to insert/update the `active_plans` row and emit the lifecycle SSE events. Keeps the tool contract LLM-facing (Claude treats it like any other tool) while the DB side is fully server-controlled.
- **Finalize-on-error is load-bearing.** BOTH the happy path AND the catch block run `finalizePlan(...)`. An exception that skips finalize leaves an `active_plans` row stuck at `status='running'` forever — the next page load rehydrates it as a zombie that blocks every send via the client-side plan send-gate. This was the actual bug behind the "stuck Eye + confusing toast" state during Session 1 bring-up.
- **Stop is two-mode by design.** `/chat-stop` reads the row's `started_at` first. If it's recent (< 30s), the endpoint only sets `requested_stop=true` and lets the live `chat.ts` function poll the flag at its next iteration boundary and finalize cleanly via the existing path. If the row is older than 30s, it force-terminates with `status='stopped'` directly — the chat function that started it is almost certainly dead, so waiting on a poll would hang forever. The client's `requestStop` rehydrates on `mode: 'force'` responses to reconcile.
- **Auto-expire stale running plans on rehydrate** (10 min threshold). `planStore.rehydrateOnLoad` marks any `status='running'` row older than the threshold as `status='failed'` with an auto-expire summary, then surfaces it as a HistoricalPlanChip rather than a live card. Defense-in-depth if finalize-on-error ever fails silently.
- **`MAX_ITERATIONS = 40`, nudge at MAX-3.** Sonnet 4.6 tends to serialize tool calls one-per-iteration unless explicitly pushed to batch. The wrap-up message at iteration 37 is phrased as a **batching nudge**, not a "stop and summarize" command — a hard-stop instruction fires too eagerly and produces "partial progress, want me to continue?" responses even when the model has room to finish.
- **Content as source of truth, UI as ephemeral view.** The `steps` jsonb + `completion_summary` text are durable; per-tool live activity (`tool_started`/`tool_finished` SSE events) is ephemeral — never persisted. Rehydrated historical plans show their step states + completion summary, but the tool-activity feed is runtime-only. Matches the same "log the decision, not every keystroke" discipline as memory.
- **DEFAULT_SYSTEM_PROMPT lives in `src/types/agentSettings.ts`** and is imported by `netlify/functions/chat.ts` rather than duplicated. Prevents drift between the server-seeded path (new org) and the client-seeded path (Settings UI lazy-upsert) — the prompt is agent behavior; silent divergence between the two seed paths produces silent divergence between workspaces.
- **Per-step tool activity: single sticky slot, not matcher-routed.** `planStore.currentTool` is one CurrentTool value overwritten on every `tool_started`; never cleared by `tool_finished`. The UI shimmers it under every active step. Known compromise: when multiple steps are simultaneously active, the same summary replicates under each. Matcher-based routing attempts (v3-v5 during bring-up) produced invisible or stale activity on fast tool calls — visibility wins over per-step precision.
- **Tool summaries are humanized server-side** via `formatToolSummary(name, input)` in `_lib/tools/toolSummary.ts`. Client never touches raw tool input — it just renders the pre-computed `"Adding Joe Rogan"` / `"Drafting Garage Cleanout · $800"` string. Keeps the client decoupled from tool schema changes.
- **Historical receipt, not historical noise.** `HistoricalPlanChip` collapses completed plans into a single-line chip (`✓ Plan complete · 4/4 ▸`) interleaved into the message stream by `completedAt` — each chip stays anchored to the moment its plan finished rather than floating at the chat tail. Click to expand for full step states + tool log with timings. Only the most recent chip pins inline for now; older ones live in the Debug tab.
- **Send-gate is in-context, not a toast.** When the user tries to send during a running plan, `pulseStopHint` bumps a nonce: MessageList force-scrolls to the tail, PlanCard's Stop button halo-pulses, ChatInput shows an inline `● Eidrix is working — wait or tap Stop above` hint. No free-floating banner; the explanation is where the user is already looking.

**Locked decisions (AC-05 Session 2 — sigils, per-iteration tool batches, plan trace):**
- **Sigils as the Eye's expressive layer.** The Eye doesn't grow, doesn't change shape, doesn't sprout anatomical limbs. It conjures a small SVG gesture in the space beside itself and dismisses it. Plan-start traces an inward spiral, complete draws a checkmark, stopped draws a halt circle, failed draws an X that fizzles before completing. The `sigilStore` is queue-based (`fire(kind)` → renderer drains via `dismiss(id)`), so any component can trigger a sigil — click acknowledgments, notifications, system events — without touching the Eye's AC-08a state machine. Path + timing for each sigil lives in one file (`sigils.ts`); adding a new gesture is ~5 lines of data. The locked brand premise: gestures are ephemeral and geometric, not structural. The Eye stays a precision instrument; sigils are what it marks the world with.
- **Sigil dispatch is one-way from planStore.** A module-level subscription in `sigilStore.ts` watches plan lifecycle transitions (null→running, running→complete/stopped/failed) and fires sigils. No planStore → sigilStore coupling inside planStore itself. If the sigil system is removed, planStore doesn't care. Clean separation.
- **Per-iteration tool batches replace single-slot currentTool.** `planStore.currentBatch: CurrentTool[]` accumulates all tools fired within the current Anthropic iteration and resets when a new iteration's first tool starts. Between iterations, the batch stays visible (sticky) so activity doesn't blink off while Claude streams text. When the agent batches 4 parallel `addCustomer` calls in one turn, all 4 render as stacked shimmer sub-lines. Solves Session 1's "not firing off all the real tool calls" observation without re-introducing the dynamic-routing bug that killed v5 — batch identity is frozen at start-of-iteration.
- **Plan trace view in the Debug tab.** Nested presentation: step list with final statuses (✓/◉/○/✗) on top, then tool calls grouped by iteration below (iteration grouping because we have no server-side tool→step linkage, and inventing one via timestamp windowing produces wrong attribution when the agent batches across step boundaries). `emitPlanStep` signaling calls are filtered out of the per-iteration view — they're already represented up top. Rendered only when `activePlanId` is set; simple single-tool turns fall through to the flat tool-trace section.
- **DebugEntry carries a server-sourced plan summary** (`activePlanId` + `activePlanSteps`). These ride on the `eidrix_usage` SSE event at end-of-stream, not reconstructed client-side. Matches the "server owns plan lifecycle" discipline from Session 1 — observability shouldn't derive facts the server already authoritatively knows.
- **Historical chips interleave across the full conversation timeline, not just the tail.** MessageList builds a unified TailItem array from `messages` + `historicalPlans` sorted by timestamp, with a 2s tiebreaker that puts assistant messages before plan-complete chips (the plan's `completedAt` lands a few hundred ms before the assistant message's persisted `created_at`). Scrollback shows every chip at its historical position. Up to 20 rehydrated plans per conversation.

**Still deferred to real-Eidrix work (NOT built in AC-05):**
- **Continue-from-where-we-left-off.** If a plan hits the iteration cap and finalizes as `failed`, there's no "resume" button. User has to manually re-prompt. Real feature with real UX complexity — what if the world changed between stop and resume?
- **Nested sub-plans.** Explicitly deferred in the chapter.
- **Concurrent plans per user.** Explicitly deferred in the chapter.
- **Plan templates** for recurring operations ("end-of-week summary", "new-customer onboarding"). Saved plans the user or agent can invoke.
- **Per-tenant plan visibility settings.** Same as memory: some orgs may want shared plans (team sees what each member asked the agent to do), others want strict isolation. Trial and Error is user-scoped; real Eidrix exposes the setting.
- **Real queue for long-running plans.** Netlify functions have a 15-min max budget even as background. Plans that legitimately need longer (batch imports, multi-hour scheduling work) need Inngest / Trigger.dev / pg-cron queue. Same graduation path as memory extraction.
- **Server-side tool→step annotation.** Could eliminate the need to group tools by iteration in the Debug trace — if `chat.ts` tagged each `eidrix_tool_started` event with the `activePlanStepId` of the most recently-active step, the trace could show tools under their actual step. Not worth it today (iteration grouping is honest and clear); worth reconsidering at scale if the trace view gets used heavily.
...
- **April 23, 2026** — AC-05 Session 1 (Multi-Turn Agentic Loops) shipped. `active_plans` table with running/complete/stopped/failed status enum, user-scoped RLS, partial index on running. `emitPlanStep` as a signaling tool — executor returns minimal ack; `chat.ts` intercepts to mutate the plan row + emit five custom SSE events (plan_started/step/complete, tool_started/finished). Finalize-on-error in both happy and catch paths — exceptions leave zombie rows otherwise. `/chat-stop` is two-mode (live-signal for fresh rows, force-terminate for stale zombies). Rehydrate auto-expires plans older than 10 min. MAX_ITERATIONS bumped to 40 with a batching-nudge warning at MAX-3. Plan UI: live PlanCard with ember left-rule, status-glyph step list, sticky single-slot shimmer sub-line under each active step showing the most recent tool activity. HistoricalPlanChip collapses completed plans into clickable receipts that expand to show step states + tool log with timings. In-context send-gate replaces the floating toast. DEFAULT_SYSTEM_PROMPT now imported from `src/types/agentSettings.ts` into `chat.ts` (single source of truth). Known compromise: single-slot current-tool design means same summary replicates under simultaneously-active steps — matcher-based routing fights visibility in practice. The agent now plans visibly, executes multi-step work, and leaves a readable trail.
- **April 24, 2026** — AC-05 Session 2 shipped, completing AC-05. Sigil system replaces "Eye grows / Eye sprouts arm" choreography ideas — the Eye conjures small SVG gestures in the space beside itself (inward spiral on plan start, checkmark on complete, halt circle on stopped, fizzling X on failed). Queue-based `sigilStore` with a `fire(kind)` API any component can call (extensibility: future click acknowledgments, notifications, system events just call `fire` with a new kind). Traced via `stroke-dashoffset` animation on SVG paths; ~150 lines of component code covers the renderer. Parallel tool visibility restored via per-iteration batching in planStore — `currentBatch: CurrentTool[]` accumulates tools from one Anthropic turn and clears when a new iteration's first tool starts (sticky between iterations so activity doesn't blink off). Debug tab gains a nested Plan trace section: step list with final statuses + tool calls grouped by iteration (signaling `emitPlanStep` calls filtered out, real work grouped by the turn that produced it). DebugEntry carries server-sourced `activePlanId` + `activePlanSteps` rather than reconstructing client-side. The Eye's character now has an expressive vocabulary — gestures are the first piece, with a clear path to add more. AC-05 complete; Eidrix is now an agent with visible presence, visible work, and visible acknowledgment.
