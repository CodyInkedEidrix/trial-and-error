...
**Complex multi-step requests that span multiple tool calls and reasoning steps** (~3-4 hours).

Example: *"Plan my Thursday"* triggers a sequence — the agent reads your schedule, identifies priorities across jobs and customers, drafts a plan, asks for confirmation, then executes the plan by creating/updating records accordingly. The agent isn't just responding to one question with one tool call; it's running a small workflow across turns.

Builds directly on AC-03's tool-calling foundation. Expect it to feel like a step-change in what the app can do — the distinction between "assistant that helps" and "agent that acts on your behalf."
